...
function highlightJobListings() {
  ...
  elements.forEach(el => {
    ...
    if (text && text.length > 80 && text.length < 30000) {
...
function detectType(text) {
  ...
  const jobContextual = [
    ...
    { pattern: /\b(key (responsibilities|duties|functions|tasks))\b/i, weight: 2 },
    { pattern: /\b(position (details|overview|summary|description))\b/i, weight: 2 },
...
function isPDFPage() {
  ...
  return (
    ...
    href.endsWith('.pdf') ||
    href.startsWith('file://') && href.includes('.pdf')
  );
}
...
async function analyzePDFPage() {
  try {
    ...
    if (analysis.type === 'resume' || isLikelyResumeText(pdfText)) {
...
function isLikelyResumeText(text) {
  ...
  const resumeIndicators = [
    ...
    /\b(objective|career goal|professional summary)\b/i,
    /\b(i |my |me |mine )/gi
  ];
  let score = 0;
  ...
  if (firstPersonCount >= 10) score += 3;
  return score >= 6;
}
